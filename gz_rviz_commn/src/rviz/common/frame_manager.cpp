...
void FrameManager::setFixedFrame(const std::string & _fixedFrame)
{
  ...
  if (gui::App()) {
    gui::App()->sendEvent(
      gui::App()->findChild<gui::MainWindow *>(),
...
void FrameManager::tf_callback(const tf2_msgs::msg::TFMessage::SharedPtr _msg)
{
  ...
  for (const auto & frame : frame_ids) {
    try {
      ...
      tfTree[tf.child_frame_id] = math::Pose3d(
...
////////////////////////////////////////////////////////////////////////////////
bool FrameManager::getFramePose(const std::string & _frame, math::Pose3d & _pose)
{
  std::lock_guard<std::mutex>(this->tf_mutex_);
...
////////////////////////////////////////////////////////////////////////////////
bool FrameManager::getParentPose(const std::string & _child, math::Pose3d & _pose)
{
  std::lock_guard<std::mutex>(this->tf_mutex_);
